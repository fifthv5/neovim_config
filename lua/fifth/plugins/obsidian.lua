return {
	...
	init = function()
		local group = vim.api.nvim_create_augroup("will-obsidian-markdown", { clear = true })

		vim.api.nvim_create_autocmd("FileType", {
			group = group,
			pattern = "markdown",
			callback = function(args)
				require("will.core.obsidian").setup_markdown_buffer(args.buf)
			end,
		})
	end,
	opts = function()
		return require("will.core.obsidian").opts()
	end,
	config = function(_, opts)
		require("obsidian").setup(opts)
		require("will.core.obsidian").patch_template_substitutions()
	end,
}
